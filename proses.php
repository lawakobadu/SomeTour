...
if (isset($_POST['rate']) && is_numeric($_POST['rate'])) {
    ...
    $sql = $con->query("SELECT * FROM `rating` WHERE ipuser = '" . $ipuser . "'");

    //hitung row
    if ($sql->num_rows > 0) {
        //lakukan update jika user sudah pernah menilai
        $con->query("UPDATE `rating` SET `rate` = '" . $rate . "' WHERE `ipuser` = '" . $ipuser . "'");
    } else {
        //simpan jika user belum pernah menilai
        $con->query("INSERT INTO `rating` VALUES ('" . $ipuser . "', '" . $rate . "')");
    }
